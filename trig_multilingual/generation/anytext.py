...
def parse_args():
    ...
    parser.add_argument('--coarse_mask_path', type=str, default=None,
                        help='Optional legacy coarse mask image directory')
    parser.add_argument('--condition_cache_dir', type=str, default=None,
                        help='Directory for materialized parquet condition images')
...
def main():
    ...
    for i, sample in tqdm(enumerate(data), desc="Processing samples", unit="sample"):
        ...
        cache_dir = args.condition_cache_dir or os.path.join(args.output_path, "_condition_image_cache")
        draw_pos_path = save_condition_image(sample, os.path.join(cache_dir, language))
        if draw_pos_path is None and args.coarse_mask_path:
            draw_pos_path = get_draw_pos_path(args.coarse_mask_path, language, img_id)
        
        # Check if draw_pos file exists
        if not draw_pos_path or not os.path.exists(draw_pos_path):
