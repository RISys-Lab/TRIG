...
def prepare_img_ids_new(batch_size: int, packed_latent_height_layout: int, packed_latent_width_layout: int,packed_latent_height_img:int, packed_latent_width_img:int,position_lists: list):
    all_img_ids_layout = []
    for position_list in position_lists:
        ...
        layout_width = packed_latent_width_layout#256//16
        layout_height =  packed_latent_height_layout#3*64//16
        #n = 2
        img_ids_layout = torch.zeros(layout_height, layout_width, 3)
        print('img_ids_layout',img_ids_layout.shape)
        if random.random() < 0:
            img_ids_layout[..., 1] = img_ids_layout[..., 1] + torch.arange(layout_height)[:, None]
            img_ids_layout[..., 2] = img_ids_layout[..., 2] + (torch.arange(layout_width)+packed_latent_width_img)[None, :]  
            ...
            for src_pts_list, dst_pts_2_list in position_list:
                ...
                # 检查边界，确保不会超出目标数组范围
                layout_h, layout_w = img_ids_np_layout.shape[:2]
                crop_h, crop_w = warped_crop.shape[:2]
                
                # 计算实际可以放置的区域
                actual_y_max = min(y_max + 1, layout_h)
                actual_x_max = min(x_max + 1, layout_w)
                actual_y_min = y_min
                actual_x_min = x_min
                
                # 如果目标区域超出边界，调整裁剪区域
                if actual_y_max - actual_y_min < crop_h or actual_x_max - actual_x_min < crop_w:
                    # 调整裁剪区域大小以匹配可用空间
                    available_h = actual_y_max - actual_y_min
                    available_w = actual_x_max - actual_x_min
                    warped_crop = warped_crop[:available_h, :available_w]
                
                img_ids_np_layout[actual_y_min:actual_y_max, actual_x_min:actual_x_max] = warped_crop
...
class FluxPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    ...
    def __init__(
        ...
        self.condition_height = 192
        self.condition_width = 256
    ...
    def prepare_latents(
            ...
            condition_height=None,
            position_file = 'data_condition/13-position.txt'
    ):
        ...
        cond_latents = self._pack_latents(image_latents, batch_size, num_channels_latents, condition_height//8, condition_width//8)
        #print('cond_latents',cond_latents.shape)
        latents = torch.concat([latents, cond_latents], dim=-2)
        with open(position_file, 'r', encoding='utf-8') as f:
            position_ = json.load(f)
        position_1 = [position_]
        latent_image_ids = prepare_img_ids_new(batch_size,condition_height//16,condition_width//16,height//2,width//2,position_1)
    ...
    def __call__(
        ...
        condition_image = self.image_processor.preprocess(condition_image, height=condition_height, width=condition_width)
        condition_image = condition_image.to(dtype=torch.float32)
        #print(type(condition_image),condition_image.shape)
        ...
        #print('latents.shape[-2]-cond_latents.shape[-2]',latents.shape[-2] - cond_latents.shape[-2],'32*32',32*32)
        # 5. Prepare timesteps
